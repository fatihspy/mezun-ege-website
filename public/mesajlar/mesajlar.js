...
router.get('/ara', authMiddleware, async (req, res, next) => {
    try {
        const aramaHam = req.query.q || '';
        const arama = aramaHam.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').substring(0, 100);
...
router.post('/:aliciId', authMiddleware, async (req, res, next) => {
    try {
        ...
        const metin = req.body.metin;
        if (!metin || typeof metin !== 'string') {
            return res.status(400).json({ basarili: false, mesaj: 'Mesaj metni zorunludur.' });
        }
        const metinTemiz = metin.replace(/[<>]/g, '').trim().slice(0, 2000);
        if (!metinTemiz) {
            return res.status(400).json({ basarili: false, mesaj: 'Geçersiz mesaj içeriği.' });
        }
        const yeniMesaj = await Mesaj.create({ gonderen: req.kullanici._id, alici: alici._id, metin: metinTemiz });
